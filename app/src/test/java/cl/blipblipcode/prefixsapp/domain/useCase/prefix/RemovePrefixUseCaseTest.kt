...
class RemovePrefixUseCaseTest {

    private lateinit var repository: FakePrefixRepository
    private lateinit var useCase: IRemovePrefixUseCase

    @Before
    fun setUp() {
        repository = FakePrefixRepository()
        useCase = RemovePrefixUseCase(repository)
    }
    ...
    fun should_propagate_prefix_to_repository_in_invoke() = runTest {
        ...
        assertEquals(prefix, repository.lastRemovedPrefix)
    }
    ...
    fun should_propagate_repository_failure_in_invoke() = runTest {
        ...
        repository.setRemovePrefixResult(Result.failure(exception))
